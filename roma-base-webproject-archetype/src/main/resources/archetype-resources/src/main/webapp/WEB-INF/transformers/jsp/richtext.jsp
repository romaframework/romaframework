...
<%
	...
	<div class="<%=JaniculumWrapper.cssClass(component, "richtext", null)%>" style="<%=JaniculumWrapper.inlineStyle(component, null)%>" id="<%=JaniculumWrapper.id(component, null)%>">
		...
		name="<%=JaniculumWrapper.fieldName(component)%>" <%=JaniculumWrapper.disabled(component)?"disabled=${symbol_escape}"disabled${symbol_escape}"":""%>
		<%
...
${symbol_dollar}('${symbol_pound}<%=JaniculumWrapper.id(component, "content") %>').ckeditor();
...
ed.on('key', function ( e ) {
	...
	var fun = ${symbol_dollar}('${symbol_pound}<%=JaniculumWrapper.id(component, "content") %>').attr("onChange");
	eval(""+fun+";");
} );
</roma:addjs>