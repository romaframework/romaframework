...
<div class="<%=JaniculumWrapper.cssClass(component, "vmenu", null)%>" style="<%=JaniculumWrapper.inlineStyle(component, null)%>" id="<%=JaniculumWrapper.id(component, null)%>" inited="false">
	<%if(JaniculumWrapper.isAction(component)){%>
		...
		<%=JaniculumWrapper.disabled(component)?"disabled=${symbol_escape}"disabled${symbol_escape}"":""%>
...
if(!isRoot){ %>
<roma:addjs>
${symbol_dollar}('${symbol_pound}<%=JaniculumWrapper.id(component,null)%>').mouseenter(function (){
	var par = ${symbol_dollar}('${symbol_pound}<%=JaniculumWrapper.id(component,null)%>');
	var ch = ${symbol_dollar}('${symbol_pound}<%=JaniculumWrapper.id(component, "content_children")%>');
...
${symbol_dollar}('${symbol_pound}<%=JaniculumWrapper.id(component,null)%>').mouseleave(function (){
	${symbol_dollar}('${symbol_pound}<%=JaniculumWrapper.id(component, "content_children")%>').css("display","none").css("z-index","0");
});
${symbol_dollar}('${symbol_pound}<%=JaniculumWrapper.id(component,"content_children")%>').css("display","none");
</roma:addjs>
<%} %>