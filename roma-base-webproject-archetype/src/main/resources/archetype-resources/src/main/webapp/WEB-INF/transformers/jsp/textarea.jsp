...
%>
	...
	if(field.isSettedFeature(ValidationFieldFeatures.MAX)){
		...
		if(max!=null && max>0){
			%>
			<roma:addjs>
			jQuery('${symbol_pound}<%=JaniculumWrapper.id(component, "content") %>').keyup(function(){
			    value = ""+jQuery('${symbol_pound}<%=JaniculumWrapper.id(component, "content") %>').val();
			    if(value.length> <%=max %>){
			        jQuery('${symbol_pound}<%=JaniculumWrapper.id(component, "content") %>').val(value.substring(0, <%=max %>));
			        romaFieldChanged('${symbol_dollar}{janiculum.fieldName()}');
			    } 
			});
			</roma:addjs>
			<%
