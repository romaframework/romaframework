...
<%if (!("raw".equals(part) || "label".equals(part))){   %>
	...
	<%if(part==null || "".equals(part) || "all".equals(part)){ %>
		...
		name="<%=JaniculumWrapper.fieldName(component)%>" value="<%=JaniculumWrapper.formatNumberContent(component)==null?"":JaniculumWrapper.formatNumberContent(component)%>" <%=JaniculumWrapper.disabled(component)?" disabled=${symbol_escape}"disabled${symbol_escape}"":""%> 
		<%
...
buffer.append("jQuery(${symbol_escape}"${symbol_pound}"+JaniculumWrapper.id(component, "content")+"${symbol_escape}").keyup(function(component){");
buffer.append("re = /[^0-9${symbol_escape}${symbol_escape}.,]/ ;");
buffer.append("re2 = /[^0-9${symbol_escape}${symbol_escape}-]/ ;");
buffer.append("var beginning = jQuery(this).attr(${symbol_escape}"value${symbol_escape}").substring(0,1);");
buffer.append("while(beginning.match(re2)){");
buffer.append("beginning = beginning.replace(re2, ${symbol_escape}"${symbol_escape}");");
buffer.append("}");
buffer.append("var end = jQuery(this).attr(${symbol_escape}"value${symbol_escape}").substring(1);");
buffer.append("while(end.match(re)){");
buffer.append("end = end.replace(re, ${symbol_escape}"${symbol_escape}");");
buffer.append("}");
buffer.append("jQuery(this).attr(${symbol_escape}"value${symbol_escape}", beginning+end);");
buffer.append("}).keyup();");
