...
value="<%=JaniculumWrapper.formatDateContent(component)%>" <%=JaniculumWrapper.disabled(component)?"disabled=${symbol_escape}"disabled${symbol_escape}"":""%>
<%
	boolean existsChangeEvent=false;
	for(String event: JaniculumWrapper.availableEvents(component)){
		if(!"change".equals(event)){
...
value="<%=JaniculumWrapper.formatDateContent(component, "HH:mm:ss")%>" <%=JaniculumWrapper.disabled(component)?"disabled=${symbol_escape}"disabled${symbol_escape}"":""%>
<%
...
%>
<roma:addjs>
jQuery('${symbol_pound}<%=JaniculumWrapper.id(component, "content") %>').datepicker({ dateFormat: 'dd/mm/yy' });
jQuery('${symbol_pound}<%=JaniculumWrapper.id(component, "time") %>').timeEntry({spinnerImage: '<%=JaniculumWrapper.contextPath() %>/static/themes/default/image/spinnerDefault.png', show24Hours: true, showSeconds: true});
</roma:addjs>
</div>